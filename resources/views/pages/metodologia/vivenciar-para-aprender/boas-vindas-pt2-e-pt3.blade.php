@extends('layouts.front-mixed')
@section('content')
<article id="content">
								<header>
									<h2 align="center"> Boas Vindas - PT2 E PT3 </h2>
								</header>
								<p>
									Objetivo: o ano de 2019 acaba de iniciar, e agora é tudo novo de novo! Vamos acolher cada um que chegar, todos que quiserem são bem vindos, e os que não quiserem vão se integrar nessas voltas as aulas 2019, pois esse ano promete muita felicidade e paz! O principal objetivo é que possamos acolher a todos os alunos, que eles saibam onde estão e para que estão! Que conheçam o ambiente escolar de uma maneira que se sintam em casa. Falando em casa, a mesma lembra organização, aconchego e liberdade, na nossa casa podemos fazer o que quiser (claro desde que tenhamos limites), nada melhor do que ver nossa casa limpa e organizada, assim sabemos onde cada coisa pode ser encontrada. Sendo assim a proposta é fazer com que ao final do projeto os alunos saibam tudo sobre a escola que da qual eles fazem parte e que conheçam cada detalhe e possam se sentir felizes e acolhidos.
								</p>
								<p>
									<b>Período:</b> 25/01 á 15/02
								</p>
								<h3>
									Turmas: P.T.2 E P.T.3
								</h3>
								<p>
									Os alunos que apresentam o perfil do p.T.2 e p.T.3, são alunos que estão em desenvolvimento das habilidades cognitivas e subsequente pedagógicas. Esses alunos na maioria das vezes apresentam déficit em atenção e concentração, além de dificuldades em memorização, a maioria apresentam vontade de aprender porém falta de estimulo e/ou limitações. Nessa fase precisamos esgotar recursos, ser criativos, para que possamos ensinar de uma maneira que eles aprendam, sintam-se capazes e curiosos. Estes alunos estão em fase de descobertas, na sua maioria possuem um verbal que pode ser nosso instrumento facilitador para alcançarmos nossos principais objetivos. É importante ressaltar as questões de estabelecimento de regras e limites, pois muitos apresentam essa dificuldade nos impedindo de apropriar-se de alguns ganhos. As vivências devem ser o carro chefe da rotina desses alunos, pois eles necessitam o tempo todo do concreto para abstrair as informações, e poder iniciar os registros. Não podemos esquecer de sempre mantê-los organizados para poder estruturar e facilitar a aprendizagem. É importante que o pedagogo organize a sua rotina em sala incluindo sempre a acolhida, a vivência, sequência de atividades, lanche, higiene, jogos, brincadeiras, o cuidar de si e do próximo e finalização do dia.
								</p>
								<p>
									Temas:
								</p>
								<ul>
									<li>
										Meu nome é
									</li>
									<li>
										Gosto e não gosto
									</li>
									<li>
										Na minha escola têm
									</li>
									<li>
										Meus pertences
									</li>
									<li>
										Calendário
									</li>
									<li>
										O som das vogais (estímulo a longo prazo)
									</li>
									<li>
										Musica das letras ( estímulo a longo prazo)
									</li>
									<li>
										Categorias (animais, pessoas, objetos, meios de transportes, alimentos, vestimentas, sentimentos e cores, tudo deve ser trabalhado ordenadamente, e conteúdo de estímulo a longo prazo)
									</li>
									<li>
										Culinária
									</li>
								</ul>
								<h3>Dicas
								</h3>
								<p>
									“Perfil do aluno”- enviar para casa para que possamos colher dados que ainda não conhecemos dos nossos alunos, bem como coisas que acalmam, comida que não gosta, restrições alimentares, medicação, o que realmente irrita ,entre outras informações importantes para uma excelente criação de vinculo, além de podermos utilizar como recurso avaliativo ao final do semestre podendo refazer e mostrar as evoluções.
								</p>
								<p>
									“Rotina”- a rotina diária de forma ilustrativa é de extrema importância para compreensão do que acontecerá ao longo do período.
								</p>
								<p>
									“Estabelecimento do quadro de regras e limites”- confeccionar com os próprios alunos já no retorno das aulas, e essas regras deverão permear todo o semestre letivo.
								</p>
								<p>
									”Calendário”- é importante que desde o primeiro dia possamos ter o calendário para eles visualizarem e registrarem para organizar os acontecimentos e compreender as questões de tempo. (nas folhas de registro, e da vivencia é importante constar sempre a data)
								</p>
								<p>
									 “Roda de conversa” – ainda que tenhamos em nossa sala alunos não verbais, podemos fazer uma roda de conversa lúdica, ou mais dinâmica para que esse vínculo se torne agradável e rico de detalhes e confiança.
								</p>
								<p>
									“Alfabeto”- confeccionar a foto de cada aluno para ser colado abaixo da letra inicial do nome, isso pode incluir o nome da professora, auxiliar e outros funcionários da escola para atingir todas as letras do alfabeto. (este fazendo parte da rotina diária da sala)
								</p>
								<p align="center">
									<b>As vivências e culinária ficam a critério dos professores, para desenvolver com Criatividade e Dinâmica, favorecendo a exploração do concreto sempre, podendo realizar vivência em duplas de professores (desde que o mesmo PT)!</b>
								</p>
								<ul>
									<li>Culinária, é necessário agendar com antecedência, avisando a coordenação para envio de bilhete</li>
								</ul>
								<p align="center">
									<b>
										“Que esse ano seja cheio de surpresas e felicidade”
									</b>
									<br> <b> Deus nos abençoe <b></br>
								</p>


							</article>
						<div class="row features">
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper first">
									<a href="#" class="image featured"><img src="images/pic03.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Boas Vindas: Ação E Organização PTS E PT1</h3>
								</header>
								<p align="justify">Objetivo: O ano de 2019 acaba de iniciar, e agora é tudo novo de novo! Vamos acolher cada um que chegar, todos que quiserem são bem </p>
								<ul class="actions">
									<li><a href="/metodologia/vivenciar-para-aprender/boas-vindas-acao-e-organizacao/" class="button">Veja mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic04.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Boas vindas: paz a gente que faz - PT 4 e PT5 </h3>
								</header>
								<p align="justify">
										Objetivo: o ano de 2019 acaba de iniciar, e agora é tudo novo de novo! Vamos acolher cada um que chegar, todos que quiserem são bem vindos,
								 </p>
								<ul class="actions">
									<li><a href="/metodologia/vivenciar-para-aprender/boas-vindas-paz-a-gente-que-faz-pt 4-ept5/" class="button">Veja mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic05.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">O outro: Quem tem um amigo tem um tesouro </h3>
								</header>
								<p align="justify">
									Objetivo: favorecer a construção da identidade a partir da interação com o outro, e essa interação pode ser construída por meio da afetividade,
								</p>
								<ul class="actions">
									<li><a href="/metodologia/vivenciar-para-aprender/o-outro-quem-tem-um-amigo-tem-um-tesouro-pts-e-pt1/" class="button">Veja mais</a></li>
								</ul>
							</section>
						</div>
					</div>
				</div>

@endsection
