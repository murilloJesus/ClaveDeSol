@extends('layouts.front-mixed')
@section('content')
<article id="content">

								<header>
									<h2 align="center"> Plano de Trabalho de Culinária </h2>
								</header>

								<p>
									Objetivo propiciar ao aluno uma atividade de descontração, troca, conhecimento,manuseio de ingredientes e a oportunidade de degustar diversos sabores, colocando em prática toda parte pedagógica.Propiciar ao aluno uma atividade voltada a tarefas caseiras, atividade diferenciada do cotidiano, determinando toda organização e cada passo a ser elaborado dentro da receita.
								</p>

								<h3>
									Conteúdos:
								</h3>

								<p>
									<b>Receita</b>
								</p>

								<ul>
									<li>
										Escolha da receita:
									</li>

									<li>
										Sorteio;
									</li>

									<li>
										Sugestões;
									</li>

									<li>
										Exposição de um livro de receita;
									</li>

									<li>
										Exposição da receita escolhida
									</li>

									<li>
										Leitura da receita;
									</li>

									<li>
										Ilustração da receita;
									</li>

									<li>
										Identificação/reconhecimento do ingrediente;
									</li>

									<li>
										Degustação de todos os ingredientes;
									</li>

									<li>
										Quantidade;
									</li>

									<li>
										Cuidado com os eletrodomésticos (garfo/ faca/ liquidificador/batedeira/forno, etc
									</li>

									<li>
										Elevar a auto-estima do aluno (sentir-se útil ao preparar uma receita);-trabalhar em equipe (aprender e respeitar as regras de convívio);-aprender bons modos à mesa;
									</li>

									<li>
										Importância que todos participem da finalização (comilança);
									</li>

								</ul>

								<h3>
									  Desenvolver:
								</h3>

								<p>
									antes de iniciar a receita os alunos devem independente da limitação física ou cognitiva, manusear, degustar, cheirar, reconhecer todos os ingredientes, explorar em todos os aspectos.
								</p>

								<h3>
									 Coordenação Motora
								</h3>

								<p>
									Proporcionar atividades como:
								</p>

								<ul>
									<li>
										misturar;
									</li>

									<li>
										bater;
									</li>

									<li>
										picar;
									</li>

									<li>
										enrolar
									</li>

									<li>
										lavar ingredientes;
									</li>

									<li>
										abrir embalagens, etc.
									</li>
								</ul>

							</article>

						<div class="row features">
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper first">
									<a href="#" class="image featured"><img src="images/pic03.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Plano de Trabalho de Socialização (PTS)</h3>
								</header>
								<p align="justify">
									Objetivo esse plano de trabalho, deve ser oferecido aos alunos que possuem baixo rendimento, e quenecessitam de muito auxílio em seus
								</p>
								<ul class="actions">
									<li><a href="/metodologia/plano-de-trabalho/plano-de-trabalho-de-socializacao-(pts)/" class="button">Veja Mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic04.jpg" alt="" /></a>
								</div>
									<header>
									<h3 align="center">Fonoaudiologia escolar - 2021</h3>
								</header>
								<p align="justify">
									O presente documento tem como objetivo apresentar as contribuições, a amplitude do fazer fonoaudiólogo no contexto educacional e
								</p>
								<ul class="actions">
									<li><a href="/metodologia/plano-terapeutico/fonoaudiologia/" class="button">Veja Mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic05.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Plano de trabalho psicologia escolar 2021 </h3>
								</header>
								<p align="justify">
									É preciso reconhecer que o processo de ensino e aprendizagem implica foco em diversas áreas do conhecimento humano, pois em meio este
								</p>
								<ul class="actions">
									<li><a href="/metodologia/plano-terapeutico/psicologia-escolar/" class="button">Próxima Pagina</a></li>
								</ul>
							</section>
						</div>
					</div>
				</div>

@endsection
