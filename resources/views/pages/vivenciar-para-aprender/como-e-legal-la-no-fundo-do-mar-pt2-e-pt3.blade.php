@extends('layouts.front')
@section('content')
	<article id="content">
								<header>
									<h2 align="center"> Reino Animal </h2>
									<p align="center">
										Como é legal lá no fundo do mar!
									</p>
								</header>
								<p>
									Objetivo: Quem convive com crianças sabem o quanto elas são fascinadas pelos diferentes animais que existem. Vamos ampliar este conhecimento separando por categorias, sendo assim iniciaremos com os ANIMAIS AQUATICOS, ao fim do projeto as crianças deverão conhecer esses animais, como eles vivem, se alimentam, suas características entre outros.
								</p>
								<p>
									<b>Período:</b> 18/02 á 29/03
								</p>
								<h3>
									Turmas: P.T.2 E P.T.3
								</h3>
								<p align="center">
									<b>
										É importante....
									</b>
								</p>
								<ul>
									<li>
										Reconhecer e identificar os diferentes tipos de animais;
									</li>
									<li>
										Promover o respeito para com todas as espécies de seres vivos;
									</li>
									<li>
										Apresentar os animais aquaticos;
									</li>
									<li>
										Desenvolver raciocínio lógico e dedutivo;
									</li>
									<li>
										Desenvolver a criatividade, a capacidade de abstração e interpretação;
									</li>
									<li>
										 Aumentar o vocabulário;
									</li>
									<li>
										 Desenvolver coordenação motora;
									</li>
									<li>
										Fazer com que as crianças identifiquem os animais existentes no fundo do mar.
									</li>
									<li>
										Conscientizar os alunos sobre os cuidados que devemos ter com o meio ambiente.
									</li>
									<li>
										Desenvolver os cuidados que devemos ter para cuidar de um peixe, alimentar, limpar o aquário.
									</li>
								</ul>
								<p>
									Temas
								</p>
								<ul>
									<li>
										Tubarão;
									</li>
									<li>
										Baleia;
									</li>
									<li>
										Cavalo-marinho
									</li>
									<li>
										Arraia;
									</li>
									<li>
										Polvo;
									</li>
									<li>
										Estrela do mar;
									</li>
									<li>
										Ostra;
									</li>
									<li>
										Tartaruga;
									</li>
									<li>
										Caranguejo;
									</li>
									<li>
										Lagosta;
									</li>
									<li>
										Corais;
									</li>
									<li>
										Calendário
									</li>
									<li>
										Culinária
									</li>
								</ul>
								<h3>
									Dicas
								</h3>
								<ul>
									<li>
										Sonde os alunos: quantos animais eles conhecem? Quantos animais diferentes tem na escola? Descubra o que eles já sabem acerca do tema;
									</li>
									<li>
										Leitura de histórias;
									</li>
									<li>
										Dobraduras;
									</li>
									<li>
										Desenho;
									</li>
									<li>
										Música
									</li>
									<li>
										Filmes;
									</li>
									<li>
										Recorte e colagem;
									</li>
									<li>
										Fazer uma maquete com os animais favoritos dos alunos;
									</li>
									<li>
										Mural sobre os animais, expondo diversidade e características que os diferenciam.
									</li>
									<li>
										Jogos e brincadeiras dirigidas;
									</li>
									<li>
										Confecção de animais com materiais recicláveis;
									</li>
									<li>
										Converse com eles sobre como todas as formas de vidas estão interligadas e dependem umas das outras, e que devemos respeitá-las e o espaço onde vivem.
									</li>
									<li>
										Confeccionar junto com os alunos <b>cartazes sensoriais/ maquetes/aquario divertido</b> que representem os animais aquaticos;
									</li>
								</ul>
								<p align="center">
									<b>
										As vivências e culinária ficam a critério dos professores, para desenvolver com Criatividade e Dinâmica, favorecendo a exploração do concreto sempre, podendo realizar vivência em duplas de professores (desde que o mesmo PT)!
									</b>
								</p>

								<ul>
									<li>
										Culinária, é necessário agendar com antecedência, avisando a coordenação para envio de bilhete sugestão: <b>culinaria que contenha atum!</b>
									</li>
								</ul>
								<h3 align="center">
									Divisão dos temas
								</h3>
								<p>
									<b>Grupos para o projeto: como é legal lá no fundo do mar – turma da manhã</b> 
								</p>
								<p>
									Grupo 1: tati e silmara - baleia
								</p>
								<p>
									Grupo 2: gizemildes e amanda - estrela do mar
								</p>
								<p>
									Grupo 3: valquíria e gislaine - caranguejo
								</p>
								<p>
									Grupo 4: luis e jeniffer – cavalo marinho
								</p>
								<p>
									Grupo 5: cris maia, nicoli e cris andrade - arraia
								</p>
								<p>
									Grupo 6: inaete, mayanne e mara dalila - tubarão
								</p>
								<p>
									Grupo 7: sueli e luciana – tartaruga e corais
								</p>
								<p>
									<b>Turmas da tarde:</b> 
								</p>
								<p>
									Grupo 1: valéria e luciana - lagosta e peixe de aquário
								</p>
								<p>
									Grupo 2: amanda sartori e gizemildes – o mar, cores e características
								</p>
							</article>

						<div class="row features">
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper first">
									<a href="#" class="image featured"><img src="images/pic03.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">O outro: Quem tem um amigo tem um tesouro <h3>
								</header>
								<p align="justify">
									Objetivo: favorecer a construção da identidade a partir da interação com o outro, e essa interação pode ser construída por meio da afetividade, 
								</p>
								<ul class="actions">
									<li><a href="#" class="button">Veja Mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic04.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Histórias carnavalescas </h3>
								</header>
								<p align="justify">
									Objetivo: o carnaval do Brasil é uma das festas mais tradicionais do mundo e desde pequenas, as crianças, escutam, vivenciam e apreciam a 
								</p>
								<ul class="actions">
									<li><a href="#" class="button">Veja Mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic05.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Educação ambiental! Preservar o meio ambiente é coisa séria! </h3>
								</header>
								<p align="justify">
									Objetivo: o assunto de preservação do meio ambiente está em alta, devido a tantas situações de riscos que a nossa natureza encontra-se. Por 
								</p>
								<ul class="actions">
									<li><a href="#" class="button">Veja Mais</a></li>
								</ul>
							</section>
						</div>
					</div>
				</div>

@endsection