@extends('layouts.front')
@section('content')
<article id="content">

								<header>
									<h2 align="center"> Plano de Trabalho de 3º ano (PT3) </h2>
								</header>

								<p>
									Objetivo:Desenvolver a capacidade crítica e as habilidades de produção espontânea, procurando avançar no desenvolvimento da escrita e da leitura e do raciocínio lógico.
								</p>
								<h3>
									Conteúdos:
								</h3>
								<h3>
									Língua Portuguesa: 
								</h3>
								<ul>
									<li>
										Reconhecimento e escrita do nome completo;
									</li>
									<li>
										Escrita espontânea;
									</li>
									<li>
										Escrita de figuras;
									</li>
									<li>
										Escrita de imagens;
									</li>
									<li>
										Redação espontânea de uma situação vivenciada;
									</li>
									<li>
										Construção de frases e pequenos textos;
									</li>
									<li>
										Continuação de um conto (imaginação);
									</li>
									<li>
										Reconhecimento e escrita dos nomes dos colegas;
									</li>
									<li>
										Expressão do pensamento e senso crítico;
									</li>
									<li>
										Jogos com letras móveis;(sorteia uma letra e o aluno deve escrever uma palavra);
									</li>
									<li>
										Recortes de palavras em jornais e revistas;
									</li>
									<li>
										Recorte de imagens que correspondam a frase;
									</li>
								</ul>
								<h3>
									Matemática:
								</h3>
								<ul>
									<li>
										Adição;
									</li>
									<li>
										Subtração;
									</li>
									<li>
										Calendário (dia/mês);
									</li>
								</ul>
								<h3>
									 Noções de:
								</h3>
								<ul>
									<li>
										Tempo *espaço *lugar/dia/noite/tamanho;
									</li>
									<li>
										Reconhecimento de números/quantidade;
									</li>
									<li>
										Sistema monetário;
									</li>
								</ul>
								<h3>
									Conhecimentos Gerais:
								</h3>
								<ul>
									<li>
										Datas comemorativas;
									</li>
									<li>
										Situações sociais;
									</li>
									<li>
										Higiene/ saúde;
									</li>
									<li>
										Meio ambiente;
									</li>
									<li>
										Direitos/deveres;
									</li>
									<li>
										Contação de histórias;
									</li>
									<li>
										Pastas das atividades do Teaach para complementos das atividades;
									</li>
									<li>
										Recortes/ colagem/ pintura;
									</li>
									<li>
										Jogos;
									</li>
									<li>
										Massinha;
									</li>
									<li>
										Passeios;
									</li>
									<li>
										Danças;
									</li>
									<li>
										Informática, usar do computador como um aliado as atividades de classe (reforço);
									</li>
									<li>
										Culinária;
									</li>
									<li>
										Ed. Física;
									</li>
									<li>
										 Musicoterapia;
									</li>
									<li>
										Visitas da Fono/ T.O./Psicóloga
									</li>
									<li>
										Brincadeiras coletivas;
									</li>
								</ul>
								<p>
									... tudo que vou ensinar , precisa ser aprendido, então cabe a mim professor buscar um meio para que essa aprendizagem venha fazer a diferença  na qualidade de vida do meu aluno!!!
								</p>
									<p>
									<b>Sinônimos de Aprendido</b>
								<br>
									<b>
										Aprendido é sinônimo de: <a href="https://www.dicio.com.br/conhecido/">conhecido</a>, <a href="https://www.dicio.com.br/sabido/">sabido</a>, <a href="https://www.dicio.com.br/entendido/">entendido</a>, <a href="https://www.dicio.com.br/desenvolvido/">desenvolvido</a>, <a href="https://www.dicio.com.br/capacitado/">capacitado</a>, <a href="https://www.dicio.com.br/treinado/">treinado</a>, <a href="https://www.dicio.com.br/fixado/">fixado</a>, <a href="https://www.dicio.com.br/gravado/">gravado</a>.
									</b>
								</br>
								</p>
							</article>

						<div class="row features">
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper first">
									<a href="#" class="image featured"><img src="images/pic03.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Plano de Trabalho de 2º ano (PT2) </h3>
								</header>
								<p align="justify">
									OBJETIVO: Esse Plano foi desenvolvido para alunos que necessitam ser estimulados cognitivamente e reorganizado para que possam vir 
								</p>
								<ul class="actions">
									<li><a href="#" class="button">Veja Mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic04.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Plano de Trabalho de 4º ano (PT4)</h3>
								</header>
								<p align="justify">
									Objetivo:Auxiliar na construção do pensamento e na expressão do mesmo com definição e propriedade. Estimular a socialização, autonomia, 
								</p>
								<ul class="actions">
									<li><a href="#" class="button">Veja Mais</a></li>
								</ul>
							</section>
							<section class="col-4 col-12-narrower feature">
								<div class="image-wrapper">
									<a href="#" class="image featured"><img src="images/pic05.jpg" alt="" /></a>
								</div>
								<header>
									<h3 align="center">Plano de Trabalho de 5º ano (PT5) </h3>
								</header>
								<p align="justify">
										Objetivo:Estimular os alunos a terem autonomia na escrita, na leitura e desenvolver atividades que promovam a estruturação da alfabetização 
							    </p>
								<ul class="actions">
									<li><a href="#" class="button">Veja Mais</a></li>
								</ul>
							</section>
						</div>
					</div>
				</div>
@endsection